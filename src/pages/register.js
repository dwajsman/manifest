...
import FirebaseContext, { UserContext } from "../context/user-provider";
        ...
        <div className="relative flex flex-row justify-center align-middle min-h-screen">
          ...
          <div className="w-x_loginContainer h-y_loginContainer space-y-8 bg-white mt-16 rounded">
            <div className="mt-87 px-417">
              ...
              <form className="mt-8 space-y-6" action="#" method="POST" onSubmit={handleSubmit(_handleLogin)}>
                  ...
                  <div className="mb-6">
                    ...
                    <input
                      ...
                      {...register('password', {
                        required: true,
                        pattern: /(?=^.{8,}$)((?=.*\d)|(?=.*\W+))(?![.\n])(?=.*[A-Z])(?=.*[a-z]).*$/,
                        validate: value => value.length < 6 ? 'Password must be at least 8 characters long' : true
                      })}
                    />
                  </div>
                </div>
